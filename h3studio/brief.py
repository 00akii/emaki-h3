...
# 載せていないもの: ダッチアングル（3回とも傾かず）/ 真俯瞰・虫の視点（1段手前で頭打ち）/
# 望遠の圧縮・広角のパース（記述を具体化しても出ない）。選んでも変わらないものは置かない。
FRAMING_PRESETS = [
    # サイズ — 本文が部屋全体を描写していると負ける。寄りで動かない画は build() が本文を絞る
...
def check_framing(framing: str, camera: str) -> dict:
    ...
    開始の構図と終端の指示の組み合わせを判定する。実測に基づく分岐:
      ...
      - 開始が寄りで、終端が静止・さらに寄る・横移動 → 修正。本文を画角内の要素に絞らないと引きに負ける（shot_size ×9 → ×0）
      - 開始が寄りで、終端が引き・舐め上げ → 警告。本文を絞ると終端の描写が足りなくなる。逃げ場がない
    ...
    if kind in ("static", "lateral") or (kind == "tighten" and (end is None or end <= size)):
        return dict(base, action="fix",
                    reason="開始が寄りで、カメラが引かない。本文を画角内の要素（表情・髪・布地・手元・肌に当たる光・背後の面の質感）に絞り、場所の記述は背景のヒントに下げる")
    if kind in ("widen", "tilt") or (kind == "tighten" and end is not None and end > size):
        return dict(base, action="warn",
                    reason="開始が寄りなのにカメラが引く（または足元から舐め上げる）。冒頭を寄りにするには本文から部屋を消す必要があり、消すと終端に書くものが無くなる。開始を引きにするか、終端を寄りに変えるのが安全")
...
def build(mode: str, fields: dict, proj: dict, selected_images: list[str],
    ...
    if mode == "A":
        ...
        if framing:
            out.append("FRAMING: %s%s" % (framing, ("。" + FRAMING_FIX_NOTE) if chk["action"] == "fix" else ""))
        if cam:
