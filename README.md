...
7. **プレビュー生成**（608×352・**実測 2.5〜7分**。プロンプトが長いほど CPU のテキストエンコードが延び、UNET の再読込が入ると更に伸びる。参照なし・短いプロンプトで 2.4分、参照画像2枚・1019語で 7分）→ 結果カードにコンタクトシート 3×3・動画・計測値（フレーム数／実尺／bit_rate／音量／フレーム間差分）。気に入らなければ「seed+1 で再生成」かブリーフへ戻る。**同じ seed・同じプロンプトの再投入は ComfyUI のキャッシュがそのまま返る**（再生成されない）
...
- **コンタクトシートも送る**（任意）
- 注釈にプロンプト本文・ブリーフ・参照素材・計測値（フレーム数／実尺／bit_rate／音量／フレーム間差分）・生成時間・job id が入り、タグは `MiniMax-H3` / `絵巻H3` / 作品名 / ショットID / 本番|プレビュー / 比率 / ref2va|t2va が付く
...
- **もっと悪い形: ComfyUI が抱えた状態で LM Studio を載せると、GPU に一部しか載らず残りが CPU に置かれる。** `lms ps` の SIZE は満額（17.16GB）を表示するので**見た目では分からない**。プロンプト生成が 45秒 → 218秒（前項の ctx の罠=215秒とは**別の事件**。こちらは ctx が正しくても起きる）。**あとから ComfyUI を空けても CPU 側の重みは GPU に移らない**（載せ直しが要る）。アプリは載せ直しが要るときだけ先に ComfyUI を降ろし、**ロード前後の VRAM 増分**で載りきったかを検算して、足りなければ1回だけ載せ直す（`gpu.prepare_for_llm`）。**合計 VRAM との比較では判定できない**（ComfyUI も VRAM を持つので誤判定する）。`/system_stats` の `vram_free` も ComfyUI 自身の会計なので使えない
- **思考を切るのは `reasoning_effort: "none"`。** `chat_template_kwargs.enable_thinking:false` だけでは効かない
- **生成時間が本によって大きく変わる。同じ設定でも速いことと遅いことがある。** 2026-08-25 の実測15本（RTX 4090・電力 360W・プレビュー 608×352・192f・6step）:

  | 位置 | 合計 |
  |---|---|
  | 1本ずつ生成（毎回モデルを読み込み直す） | **126 / 128 / 133 / 142 秒**、ただし1本は **336 秒** |
  | 同じ設定で連投中（モデルが載ったまま） | **45 / 49 秒** |

  遅い本は **step 1（モデルの読み込み）だけ**が伸びる（速い本 6〜10秒 に対し 213〜494秒）。**何が分けているのかは分かっていない。**
  空き VRAM では説明できない — **同じ空き 20638 MB から step 1 が 10.28 秒のことも 212.93 秒のこともあった**（別の組でも同様）。
  観測された詰まりは**モデルを読み込み直す本でだけ**起きているが、読み込み直す本が必ず遅いわけでもない（9本中3本）。
