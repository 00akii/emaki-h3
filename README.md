...
2. **ブリーフ** を書く。推奨（4欄）が既定。各欄の `?` で書き方の説明が開く
   ...
   - **開始の構図**は1フレーム目。高さ（目線／ハイ／ロー）と視点（正面／真横／背面）は語を足すだけで効く。
     **サイズ（寄り）を選ぶと、プロンプトの描写を画角内の要素に絞る修正が自動で入る**（絞らないと描写のほうが勝って指定が黙って無視される・実測 ×9 → ×0）。
     選んだ組み合わせの判定（そのまま通す／修正が入る／成立しにくい）は欄のすぐ下に出る。
     **ダッチアングル・真俯瞰・広角も選べる**（2026-08-27 追加。選ぶと、場面全体をその効果に矛盾しない世界として描くよう描写が自動で切り替わる。サイズの修正と同じ仕組み）。**ただし傾く向き・角度・誇張の強さは選べない。**
     **望遠の圧縮だけは選択肢に置いていない**（本文ごと書き換えても効かないことが実測で確認済み）
...
- **生成にかかる時間は、1回ごとに大きく変わる。同じ設定でも速いときと遅いときがある。** 2026-08-25 の実測15本（RTX 4090・電力 360W・プレビュー 608×352・192f・6step）:
  ...
  **126 / 128 / 133 / 142 秒**、ただし1回だけ **336 秒**。

  **⚠ 以前ここには「モデルを載せたまま続けて生成すると45/49秒」という表もあったが、2026-08-26 に削除した。**
  その状態（`config.json` の `gen.vram_mode: "auto"` が選んでいた `resident`）は purge を挟まないため VRAM が詰まりやすく、
  生成成功直後の再生成で完了も中止もできなくなる事故が実測7本中2本＋別途2本で起きた。**`auto` は常に `share` に変更したので、この状態自体がもう起きない。**
  （明示的に `vram_mode: "resident"` を選べば同じ詰まりを踏める。実験用に残してあるが推奨しない）
...
- **⚠ `--reserve-vram 3` は効かなかった。** 以前この README は「筋の良い対処」として推奨していたが、**2026-08-25 に実測して撤回した。**
  付けて起動した状態でも**詰まりは出る**（step 1 が 494 秒）し、**完了も中止もできない状態にもなる**
  （`vram_mode: "resident"` を使っていた本で発生。この設定は上の理由で 2026-08-26 に既定から外した）。
...
- **権利侵害やライセンス上の懸念に気づいたら、[docs/報告窓口.md](docs/%E5%A0%B1%E5%91%8A%E7%AA%93%E5%8F%A3.md) から報告してください。** 公開してよい内容は GitHub の Issue、公開したくない内容は非公開報告（Security タブの「Report a vulnerability」）が使えます。
